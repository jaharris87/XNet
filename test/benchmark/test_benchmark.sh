...
copy short; sed 's/^repetitions_requested\t.*/repetitions_requested\t2/' "$tmp/short/metadata.tsv" > "$tmp/short/meta"; mv "$tmp/short/meta" "$tmp/short/metadata.tsv"; reject short-rows sh "$root/validate.sh" "$tmp/short"
