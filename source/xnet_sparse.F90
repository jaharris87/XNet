...
Contains
  ...
  Subroutine validate_record_sizes(file_name,data,status,io_status,message)
    ...
    Integer :: lun_probe, short_record_status(11)
    ...
    Call determine_short_record_status(data,probe_data,short_record_status,status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    ...
    Call finish_record_probe(lun_probe,'header record',short_record_status(1),status,io_status, &
      & message)
    If ( status /= sparse_ind_ok ) Return
    ...
    Call finish_record_probe(lun_probe,'topology record',short_record_status(2),status,io_status, &
      & message)
    If ( status /= sparse_ind_ok ) Return
    ...
    Call finish_record_probe(lun_probe,'reaction-map dimension record',short_record_status(3), &
    ...
    Read(lun_probe,iostat=io_status) probe_data%ns11, probe_data%ns21, probe_data%ns22, &
      & record_probe
    Call finish_record_probe(lun_probe,'one/two-reactant map record',short_record_status(4), &
      & status, &
      & io_status,message)
    If ( status /= sparse_ind_ok ) Return
    ...
    Call finish_record_probe(lun_probe,'ns31 map record',short_record_status(5),status,io_status, &
      & message)
    If ( status /= sparse_ind_ok ) Return
    ...
    Call finish_record_probe(lun_probe,'ns32 map record',short_record_status(6),status,io_status, &
      & message)
    If ( status /= sparse_ind_ok ) Return
    ...
    Call finish_record_probe(lun_probe,'ns33 map record',short_record_status(7),status,io_status, &
      & message)
    If ( status /= sparse_ind_ok ) Return
    ...
    Call finish_record_probe(lun_probe,'ns41 map record',short_record_status(8),status,io_status, &
      & message)
    If ( status /= sparse_ind_ok ) Return
    ...
    Call finish_record_probe(lun_probe,'ns42 map record',short_record_status(9),status,io_status, &
      & message)
    If ( status /= sparse_ind_ok ) Return
    ...
    Call finish_record_probe(lun_probe,'ns43 map record',short_record_status(10),status,io_status, &
      & message)
    If ( status /= sparse_ind_ok ) Return
    ...
    Call finish_record_probe(lun_probe,'ns44 map record',short_record_status(11),status,io_status, &
      & message)

    Return
  End Subroutine validate_record_sizes

  Subroutine determine_short_record_status(data,probe_data,short_record_status,status, &
    & io_status,message)
    !-----------------------------------------------------------------------------------------------
    ! Sequential-unformatted short-record errors have processor-specific positive status values.
    ! Generate the condition on a fresh scratch unit with the identical transfer list for each
    ! persisted record so each production probe can require its corresponding processor value.
    !-----------------------------------------------------------------------------------------------
    Implicit None

    Type(sparse_data), Intent(in) :: data
    Type(sparse_data), Intent(inout) :: probe_data
    Integer, Intent(out) :: short_record_status(11), status, io_status
    Character(*), Intent(out) :: message
    ...
    Call start_short_record_calibration(lun_probe,status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    Write(lun_probe,iostat=io_status,iomsg=io_message) data%lval
    Call prepare_short_record_calibration(lun_probe,'header record',status,io_status,message, &
      & io_message)
    If ( status /= sparse_ind_ok ) Return
    Read(lun_probe,iostat=short_record_status(1)) probe_data%lval, record_probe
    Call finish_short_record_calibration(lun_probe,'header record',short_record_status(1), &
      & status,io_status,message)
    If ( status /= sparse_ind_ok ) Return

    Call start_short_record_calibration(lun_probe,status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    Write(lun_probe,iostat=io_status,iomsg=io_message) data%ridx, data%cidx, data%pb
    Call prepare_short_record_calibration(lun_probe,'topology record',status,io_status,message, &
      & io_message)
    If ( status /= sparse_ind_ok ) Return
    Read(lun_probe,iostat=short_record_status(2)) probe_data%ridx, probe_data%cidx, &
      & probe_data%pb, record_probe
    Call finish_short_record_calibration(lun_probe,'topology record',short_record_status(2), &
      & status,io_status,message)
    If ( status /= sparse_ind_ok ) Return

    Call start_short_record_calibration(lun_probe,status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    Write(lun_probe,iostat=io_status,iomsg=io_message) data%l1s, data%l2s, data%l3s, data%l4s
    Call prepare_short_record_calibration(lun_probe,'reaction-map dimension record',status, &
      & io_status,message,io_message)
    If ( status /= sparse_ind_ok ) Return
    Read(lun_probe,iostat=short_record_status(3)) probe_data%l1s, probe_data%l2s, &
      & probe_data%l3s, probe_data%l4s, record_probe
    Call finish_short_record_calibration(lun_probe,'reaction-map dimension record', &
      & short_record_status(3),status,io_status,message)
    If ( status /= sparse_ind_ok ) Return

    Call start_short_record_calibration(lun_probe,status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    Write(lun_probe,iostat=io_status,iomsg=io_message) data%ns11, data%ns21, data%ns22
    Call prepare_short_record_calibration(lun_probe,'one/two-reactant map record',status, &
      & io_status,message,io_message)
    If ( status /= sparse_ind_ok ) Return
    Read(lun_probe,iostat=short_record_status(4)) probe_data%ns11, probe_data%ns21, &
      & probe_data%ns22, record_probe
    Call finish_short_record_calibration(lun_probe,'one/two-reactant map record', &
      & short_record_status(4),status,io_status,message)
    If ( status /= sparse_ind_ok ) Return

    Call calibrate_map_record(data%ns31,probe_data%ns31,'ns31 map record', &
      & short_record_status(5),status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    Call calibrate_map_record(data%ns32,probe_data%ns32,'ns32 map record', &
      & short_record_status(6),status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    Call calibrate_map_record(data%ns33,probe_data%ns33,'ns33 map record', &
      & short_record_status(7),status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    Call calibrate_map_record(data%ns41,probe_data%ns41,'ns41 map record', &
      & short_record_status(8),status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    Call calibrate_map_record(data%ns42,probe_data%ns42,'ns42 map record', &
      & short_record_status(9),status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    Call calibrate_map_record(data%ns43,probe_data%ns43,'ns43 map record', &
      & short_record_status(10),status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    Call calibrate_map_record(data%ns44,probe_data%ns44,'ns44 map record', &
      & short_record_status(11),status,io_status,message)

    Return
  End Subroutine determine_short_record_status

  Subroutine calibrate_map_record(map,probe_map,record_name,short_record_status,status, &
    & io_status,message)
    Implicit None

    Integer, Intent(in) :: map(:)
    Integer, Intent(inout) :: probe_map(:)
    ...
    Call start_short_record_calibration(lun_probe,status,io_status,message)
    If ( status /= sparse_ind_ok ) Return
    Write(lun_probe,iostat=io_status,iomsg=io_message) map
    Call prepare_short_record_calibration(lun_probe,record_name,status,io_status,message, &
      & io_message)
    If ( status /= sparse_ind_ok ) Return
    Read(lun_probe,iostat=short_record_status) probe_map, record_probe
    Call finish_short_record_calibration(lun_probe,record_name,short_record_status,status, &
      & io_status,message)

    Return
  End Subroutine calibrate_map_record

  Subroutine start_short_record_calibration(lun,status,io_status,message)
    Implicit None

    Integer, Intent(out) :: lun, status, io_status
    Character(*), Intent(out) :: message
    ...
    io_status = 0
    message = ''
    Open(newunit=lun,status='scratch',action='readwrite',form='unformatted', &
      & iostat=io_status,iomsg=io_message)
    If ( io_status /= 0 ) Call read_failure(status,message,'record-size probe setup',io_message)

    Return
  End Subroutine start_short_record_calibration

  Subroutine prepare_short_record_calibration(lun,record_name,status,io_status,message,io_message)
    Implicit None
    ...
    status = sparse_ind_ok
    message = ''
    If ( io_status == 0 ) Rewind(lun,iostat=io_status,iomsg=io_message)
    If ( io_status /= 0 ) Then
      Call read_failure(status,message,trim(record_name)//' calibration setup',io_message)
      Close(lun)
    EndIf

    Return
  End Subroutine prepare_short_record_calibration

  Subroutine finish_short_record_calibration(lun,record_name,short_record_status,status, &
    & io_status,message)
    Implicit None

    Integer, Intent(in) :: lun, short_record_status
    Character(*), Intent(in) :: record_name
    Integer, Intent(out) :: status, io_status
    Character(*), Intent(out) :: message
  ...
  Subroutine finish_record_probe(lun,record_name,short_record_status,status,io_status,message)
    ...
    ElseIf ( io_status /= short_record_status ) Then
      Call read_failure(status,message,trim(record_name)//' size probe', &
        & 'unexpected input status while checking record size')
    Else
      ! This processor returned the same error for the identical scratch-record transfer list.
      io_status = 0
    EndIf
