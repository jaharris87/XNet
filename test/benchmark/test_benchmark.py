...
def main(record: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="xnet-v9-benchmark-tests-") as temporary:
        ...
        missing = copy("missing"); (missing / "input-bundle").rmdir() if not any((missing / "input-bundle").iterdir()) else shutil.rmtree(missing / "input-bundle"); reject("missing bundle", missing, root)
        ...
        tamper = copy("tamper"); target = next(path for path in (tamper / "input-bundle").rglob("*") if path.is_file()); target.write_bytes(target.read_bytes() + b"x"); reject("input tamper", tamper, root)
