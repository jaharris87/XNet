...
This manually initiated check qualifies one clean XNet commit on one Frontier
MI250X GPU. It is not CI, a
...
- the ten-zone, `nzbatchmx=4` fixture under `test/qualification/parallel_zones/`
  runs once with the CPU executable and once with the GPU executable. The existing runner
...
Its `status` must be updated with the retained measured Frontier results and
review outcome before the qualification is accepted.
...
not silently return to native preprocessing; this procedure qualifies only
...
new bound still rejects, and a final rerun from the commit under review.
